...
class MethodCNN(method, nn.Module):
    ...
    def __init__(self, mName, mDescription):
        ...
        # check here for nn.cov2d doc: https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html
        ...
        self.fc_layer_2 = nn.Linear(16 * 4 * 4, 120)
        self.activation_func_2 = nn.ReLU()

        self.fc_layer_3 = nn.Linear(120, 10)
    ...
    def forward(self, x):
        """Forward propagation"""
        # cnn layers
        x_inter = F.max_pool2d(F.relu(self.conv1(x)), (2, 2))
        h = F.max_pool2d(F.relu(self.conv2(x_inter)), 2)
        # output layer result

        # FC layers
        h_fc_inter = torch.flatten(h)
        h = self.activation_func_1(self.fc_layer_1(h_fc_inter))
    ...
    def train(self):
        ...
        for epoch in range(self.max_epoch):
            ...
            for data in self.data['train']:
                ...
                X_train = torch.FloatTensor(np.array(X)).unsqueeze(0).unsqueeze(0)
